ALTER TABLE public.live_bids ADD COLUMN IF NOT EXISTS lot_number integer;

UPDATE public.live_bids b
SET lot_number = l.lot_number
FROM public.live_auctions l
WHERE b.live_id = l.id AND b.lot_number IS NULL;

CREATE INDEX IF NOT EXISTS live_bids_lot_timeline_idx
    ON public.live_bids (live_id, lot_number, created_at DESC);
...
BEGIN
    ...
    INSERT INTO public.live_bids (live_id,lot_number,user_id,user_name,amount) VALUES (p_live_id,v_live.lot_number,auth.uid(),v_user_name,p_amount);
