    ...
    <div className="animate-fade-up bg-white">
                ...
                <div className="mt-8 space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-slate-900 text-white font-black uppercase tracking-widest text-[9px] rounded-xl hover:bg-rose-600 transition-colors"
                    >
                      Colocar no Carrinho
                    </button>
                  </div>
      ...
      <section className="py-32 px-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
          {[
            { id: "hlt-1", name: "Pikachu Illustrator", price: 1200000, img: "https://images.pokemontcg.io/promo/1.png" },
            { id: "hlt-2", name: "Mewtwo Shadowless", price: 25000, img: "https://images.pokemontcg.io/base1/10.png" },
            { id: "hlt-3", name: "Lugia Legend", price: 8500, img: "https://images.pokemontcg.io/hgss1/114.png" }
          ].map((card, i) => (
            <div key={i} className="group cursor-pointer">
              <div
                onClick={() => addItem({ id: card.id, name: card.name, price: card.price, imageUrl: card.img })}
                className="aspect-[3/4] bg-slate-50 rounded-3xl mb-6 overflow-hidden border border-slate-100 group-hover:shadow-2xl transition-all relative"
              >
                <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-slate-900 to-transparent flex justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10">
                  <button className="bg-white text-slate-900 font-black text-[9px] uppercase tracking-widest px-6 py-2 rounded-xl">🛒 Adicionar</button>
                </div>
                <img src={card.img} alt={card.name} className="w-full h-full object-contain p-8 group-hover:scale-110 transition-transform duration-500 relative z-0" />
              </div>
              <h3 className="text-lg font-black tracking-tighter text-slate-900">{card.name}</h3>
              <p className="text-rose-600 font-black text-sm">R$ {card.price.toLocaleString('pt-BR')}</p>
            </div>
          ))}
        </div>
      </section>
