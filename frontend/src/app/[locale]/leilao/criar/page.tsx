...
export default function CreateAuctionPage() {
    ...
    const [sets, setSets] = useState<{ id: string; name: string; alias?: string; }[]>([]);
    ...
    React.useEffect(() => {
        // Flatten EDITIONS into a single list of sets for the dropdown
        const allSets = EDITIONS.flatMap(yearData => 
            yearData.sets.map(([name, id, alias]) => ({ id, name, alias }))
    ...
    const [setCards, setSetCards] = useState<{ id: string; name: string; local_id: string; image_url: string; }[]>([]);
    const [searchingCards, setSearchingCards] = useState(false);
    const [cardSearchQuery, setCardSearchQuery] = useState('');
    const [showCardList, setShowCardList] = useState(false);

    // Fetch cards for the selected set to populate the second dropdown
    React.useEffect(() => {
        const fetchSetCards = async () => {
            ...
            try {
                const { data, error } = await supabase
                    .from('pokemon_cards')
                    .select('id, name, local_id, image_url')
                    .eq('set_id', selectedSet)
                    .order('local_id');
                
                if (!error && data) {
                    setSetCards(data as any);
                }
            } finally {
                    ...
                    <div className="lg:col-span-8">
                        <form onSubmit={handleSubmit} className="space-y-10">
                            <div className="bg-white/80 backdrop-blur-xl border border-slate-200 p-8 sm:p-12 rounded-[48px] shadow-2xl shadow-slate-200/50 space-y-10 relative overflow-hidden">
                                ...
                                <div className="space-y-6 mb-8 bg-slate-50/50 backdrop-blur-md p-6 rounded-[32px] border border-slate-200/60 relative z-10 shadow-inner">
                                    <div className="flex flex-col gap-4">
                                        <div className="flex-1 relative">
                                            <div 
                                                onClick={() => setShowSets(!showSets)}
                                                className="w-full h-16 px-8 bg-white border border-transparent rounded-[24px] text-[11px] focus:outline-none focus:ring-4 focus:ring-rose-500/10 transition-all font-black text-slate-900 flex items-center justify-between cursor-pointer shadow-sm uppercase tracking-widest group"
                                            >
                                                <span className={selectedSet ? 'text-slate-900' : 'text-slate-400 opacity-60'}>
                                                    {selectedSet ? sets.find(s => s.id === selectedSet)?.name : '1. Selecionar Coleção...'}
                                                </span>
                                                <svg className={`w-4 h-4 transition-transform ${showSets ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M19 9l-7 7-7-7" /></svg>
                                            </div>

                                            {showSets && (
                                                <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-200 rounded-3xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in duration-200">
                                                    <div className="p-4 border-b border-slate-100">
                                                        <input
                                                            autoFocus
                                                            type="text"
                                                            placeholder="BUSCAR POR NOME OU SIGLA..."
                                                            className="w-full h-12 px-5 bg-slate-50 border border-slate-100 rounded-2xl text-[10px] font-black outline-none focus:border-rose-600 transition-all uppercase tracking-widest"
                                                    ...
                                                    <div className="max-h-[250px] overflow-y-auto">
                                                        <div 
                                                            className="px-6 py-4 hover:bg-slate-50 cursor-pointer text-[10px] font-black text-rose-600 border-b border-slate-50 uppercase tracking-widest"
                                                            onClick={() => { setSelectedSet(''); setShowSets(false); }}
                                                        >
                                                            Todas as Coleções
                                                        </div>
                                                        {filteredSets.map(set => (
                                                            <div 
                                                                key={set.id}
                                                                onClick={() => { setSelectedSet(set.id); setShowSets(false); setSetSearchQuery(''); }}
                                                                className={`px-6 py-4 hover:bg-slate-50 cursor-pointer text-[10px] font-black transition-colors border-b border-slate-50 last:border-0 flex items-center justify-between group uppercase tracking-widest ${selectedSet === set.id ? 'bg-rose-50 text-rose-600' : 'text-slate-700'}`}
                                                            >
                                                                <span>{set.name}</span>
                                                                <span className="text-[9px] opacity-40 font-black group-hover:opacity-100">{set.alias || set.id}</span>
                                                            </div>
                                                        ))}
                                                    </div>
                                                </div>
                                        ...
                                        {selectedSet && (
                                            <div className="flex-1 relative animate-fade-in">
                                                <div 
                                                    ...
                                                    className="w-full h-16 px-8 bg-white border border-transparent rounded-[24px] text-[11px] focus:outline-none focus:ring-4 focus:ring-rose-500/10 transition-all font-black text-slate-900 flex items-center justify-between cursor-pointer shadow-sm uppercase tracking-widest group"
                                                >
                                                    <span className={cardName ? 'text-slate-900' : 'text-slate-400 opacity-60'}>
                                                        {searchingCards ? 'CARREGANDO LISTA...' : cardName ? `${cardName} (#${cardNumber})` : '2. Escolher Carta...'}
                                                    </span>
                                                    <svg className={`w-4 h-4 transition-transform ${showCardList ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M19 9l-7 7-7-7" /></svg>
                                                </div>
                                                        ...
                                                        <div className="max-h-[300px] overflow-y-auto">
                                                            {filteredCardList.map(c => (
                                                                <div 
                                                                    ...
                                                                    className="px-6 py-4 hover:bg-emerald-50 cursor-pointer text-[10px] font-black transition-colors border-b border-slate-50 last:border-0 flex items-center justify-between group uppercase tracking-widest text-slate-700"
                                                                >
                                                                    <div className="flex items-center gap-4">
                                                                        <span className="text-[9px] font-black text-slate-400 w-10">{c.local_id}</span>
                                                                        <span>{c.name}</span>
                                                                    </div>
                                                                    <span className="text-[8px] font-black text-emerald-600 opacity-0 group-hover:opacity-100 transition-opacity">Selecionar</span>
                                                                </div>
                                                            ))}
                                                            {filteredCardList.length === 0 && (
                                                                <div className="p-10 text-center text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">Nenhuma carta encontrada</div>
                                                            )}
                                                        </div>
                                                    </div>
                                                )}
                                        ...
                                        <div className="flex flex-col md:flex-row gap-4">
                                            <div className="flex-[2] relative group">
                                                <input
                                                    type="text"
                                                    placeholder={selectedSet ? "OPCIONAL: BUSCAR POR NOME NA GALERIA..." : "BUSCAR POR NOME EM TODAS AS COLEÇÕES..."}
                                                    className="w-full h-16 px-8 bg-white border border-transparent rounded-[24px] text-[11px] focus:outline-none focus:ring-4 focus:ring-rose-500/10 transition-all font-black text-slate-900 shadow-sm group-hover:border-slate-200 uppercase tracking-widest"
                                                    value={cardName}
                                            ...
                                            <button
                                                ...
                                                onClick={searchCards}
                                                disabled={searching}
                                                className="h-16 px-10 bg-slate-950 text-white font-black uppercase tracking-widest text-[10px] rounded-[24px] hover:bg-rose-600 transition-all shadow-xl shadow-slate-900/10 disabled:opacity-50"
                                            >
                                                {searching ? '...' : 'BUSCAR IMAGENS'}
                                            </button>
                                        </div>
                                    </div>
                                </div>
