...
export default function NovaCartaPage() {
    ...
    useEffect(() => {
        const init = async () => {
            ...
            // Usa a lista estática de edições (inclui todas, inclusive as customizadas)
                ...
                {step === 1 && (
                    <div className="bg-slate-900 border border-white/5 rounded-[40px] p-8 space-y-8 shadow-sm">
                        ...
                        <div className="space-y-4">
                            {/* Coleção - custom searchable dropdown */}
                            <div>
                                <label className="text-[10px] font-black text-brand-muted uppercase tracking-widest mb-2 block">1. Coleção</label>
                                <div className="relative">
                                    ...
                                    {showSets && (
                                        <div className="absolute top-full left-0 right-0 mt-1 rounded-2xl border border-white/10 shadow-2xl z-50 overflow-hidden" style={{ background: '#0c1324' }}>
                                            <div className="p-3">
                                                <input
                                                    autoFocus
                                                    type="text"
                                                    placeholder="Buscar por nome ou sigla..."
                                                    className="w-full h-10 px-4 bg-white/5 border border-brand-rose/50 rounded-xl text-xs font-bold text-brand-text outline-none"
                                            ...
                                            <div className="max-h-[300px] overflow-y-auto">
                                                <div
                                                    onClick={() => { setSelectedSet(''); setShowSets(false); setSetSearchQuery(''); }}
                                                    className={`px-5 py-3 cursor-pointer text-xs font-bold transition-colors border-b border-white/5 flex items-center justify-between ${!selectedSet ? 'text-brand-rose bg-brand-rose/5' : 'text-brand-text hover:bg-white/5'}`}
                                                >
                                                ...
                                                {sets
                                                    ...
                                                    .map(s => (
                                                        <div
                                                            key={s.id}
                                                            onClick={() => { setSelectedSet(s.id); setShowSets(false); setSetSearchQuery(''); }}
                                                        ...
                                                        >
                                                            <span>{s.name}</span>
                                                            <span className="text-[10px] opacity-40 font-black group-hover:opacity-100 uppercase">{s.alias || s.id}</span>
                                                        </div>
                                                    ))
                                                }
                                            </div>
                            ...
                            <div className="flex gap-3">
                                <input
                                    ...
                                    className="flex-1 h-12 px-4 bg-white/5 border border-white/10 rounded-2xl text-sm font-bold text-brand-text outline-none focus:border-brand-rose/50 transition-all"
                                />
                                <button
                                    onClick={searchCards}
                                    disabled={searching}
                                    className="h-12 px-8 bg-brand-rose text-white font-black uppercase tracking-widest text-[10px] rounded-2xl hover:bg-brand-rose-dim transition-all disabled:opacity-50 shadow-lg shadow-brand-rose/20"
                                >
                                    {searching ? '...' : 'Buscar'}
                                </button>
                            </div>
