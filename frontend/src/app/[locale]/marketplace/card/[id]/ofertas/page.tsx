...
export default function CardOfertasPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const fetchListings = async () => {
            ...
            // 1. Busca dados da carta primeiro (independente dos listings)
            const { data: enrichedData } = await supabase
                .from('enriched_inventory')
                .select('card_id, name, official_name, official_image_url, image_url, card_set, card_number, rarity')
                .eq('id', decoded)
                .single();

            if (enrichedData) {
                setCardData({
                    name: enrichedData.official_name || enrichedData.name || decoded,
                    image: enrichedData.official_image_url || enrichedData.image_url || '',
                    set: enrichedData.card_set || '',
                    number: enrichedData.card_number || '',
                    rarity: enrichedData.rarity || '',
                });
            }
    ...
    // Agrupa por tipo de acabamento e calcula min/média/max
    const priceStats = (() => {
        const groups: Record<string, number[]> = {};
        listings.forEach(l => {
            const key = l.finish || 'Normal';
            if (!groups[key]) groups[key] = [];
            groups[key].push(l.price);
        });
        return Object.entries(groups).map(([finish, prices]) => ({
            finish,
            min: Math.min(...prices),
    ...
    return (
        <div className="animate-fade-up pb-20 pt-10">
                ...
                <div className="flex flex-col gap-8 lg:flex-row lg:items-start">
                        ...
                        {cardData?.image ? (
                            <div className="relative w-48 mx-auto lg:mx-0">
                                <img
                                    src={cardData.image}
                                    alt={cardData.name}
                                    className="w-full rounded-2xl shadow-2xl shadow-slate-900/20"
                                />
                                <div className="absolute inset-0 rounded-2xl ring-1 ring-inset ring-white/10" />
                            </div>
                        ) : (
                            <div className="w-48 h-64 mx-auto lg:mx-0 rounded-2xl bg-slate-100 flex items-center justify-center animate-pulse">
                    ...
                    <div className="flex-1 space-y-4">
                        ...
                        <h1 className="text-3xl font-black leading-tight tracking-tight text-slate-950 sm:text-4xl lg:text-5xl">
                            {cardData?.name || decodeURIComponent(id)}
                        </h1>
                        {cardData && (
                            <div className="flex flex-wrap items-center gap-2 mt-1">
                                {cardData.set && (
                                    <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">
                                        {cardData.set}{cardData.number ? ` · #${cardData.number}` : ''}
                                    </p>
                                )}
                                {cardData.rarity && (
            ...
            {/* Bloco de Referência de Preços */}
            {priceStats.length > 0 && (
                <section className="page-frame mt-6">
                    <div className="surface-card overflow-hidden">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/60">
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-[0.25em] text-slate-400">Referência de Preço no Marketplace</p>
                                <p className="text-xs text-slate-500 mt-0.5">{listings.length} anúncio{listings.length !== 1 ? 's' : ''} ativos considerados</p>
                            </div>
                        </div>

                        {/* Header desktop */}
                        <div className="hidden sm:grid grid-cols-[1fr_1fr_1fr_1fr] border-b border-white/5" style={{background:'#23293c'}}>
                            <div className="px-6 py-3 text-[9px] font-black uppercase tracking-[0.2em] text-brand-muted">Acabamento</div>
                            <div className="px-6 py-3 text-[9px] font-black uppercase tracking-[0.2em] text-brand-muted text-center border-l border-white/5">Menor</div>
                            <div className="px-6 py-3 text-[9px] font-black uppercase tracking-[0.2em] text-brand-muted text-center border-l border-white/5">Média</div>
                            <div className="px-6 py-3 text-[9px] font-black uppercase tracking-[0.2em] text-brand-muted text-center border-l border-white/5">Maior</div>
                        </div>

                        <div className="divide-y divide-white/5">
                            {priceStats.map(({ finish, min, avg, max, count }) => (
                                <div key={finish} className="grid grid-cols-2 sm:grid-cols-[1fr_1fr_1fr_1fr] hover:bg-white/5 transition-colors">
                                    <div className="px-6 py-4 flex items-center gap-3">
                                        <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${
                                            finish === 'Foil' ? 'bg-brand-amber' :
                                            finish === 'Reverse Foil' ? 'bg-violet-400' :
                                            'bg-brand-muted'
                                        }`} />
                                        <div>
                                            <p className="text-sm font-black text-brand-text">{finish}</p>
                                            <p className="text-[9px] font-bold text-brand-muted mt-0.5">{count} anúncio{count !== 1 ? 's' : ''}</p>
                                        </div>
                                    </div>
                                    <div className="px-6 py-4 flex flex-col justify-center sm:border-l border-white/5">
                                        <p className="text-[9px] font-black uppercase tracking-widest text-brand-muted sm:text-center">Menor</p>
                                        <p className="text-base font-black text-emerald-400 sm:text-center mt-0.5">{fmt(min)}</p>
                                    </div>
                                    <div className="px-6 py-4 flex flex-col justify-center border-l border-white/5">
                                        <p className="text-[9px] font-black uppercase tracking-widest text-brand-muted sm:text-center">Média</p>
                                        <p className="text-base font-black text-brand-text sm:text-center mt-0.5">{fmt(avg)}</p>
                                    </div>
                                    <div className="px-6 py-4 flex flex-col justify-center border-l border-white/5">
                                        <p className="text-[9px] font-black uppercase tracking-widest text-brand-muted sm:text-center">Maior</p>
                                        <p className="text-base font-black text-brand-rose sm:text-center mt-0.5">{fmt(max)}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            <section className="page-frame mt-8 space-y-6">
                <div className="surface-card flex flex-wrap items-center gap-3 p-5">
                    ...
                    <div className="ml-auto flex items-center gap-2">
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-muted">Ordenar:</span>
                        <select
                            id="sort-listings"
                            value={sortBy}
                            onChange={e => setSortBy(e.target.value as typeof sortBy)}
                            className="h-8 rounded-xl border border-white/10 bg-white/5 px-3 text-[10px] font-black uppercase tracking-widest text-brand-text outline-none cursor-pointer"
                        >
                ...
                {loading ? (
                    ...
                    <div className="surface-card flex min-h-64 flex-col items-center justify-center gap-4 p-10 text-center">
                        ...
                        {listings.length === 0 && (
                            <a href="/vender/nova-carta" className="inline-flex h-12 items-center justify-center rounded-2xl bg-brand-rose px-8 text-[10px] font-black uppercase tracking-widest text-white transition-all hover:bg-brand-rose-dim">
                    ...
                    <div className="surface-card overflow-hidden">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-white/5" style={{background:'#23293c'}}>
                            ...
                            <a href="/vender/nova-carta" className="text-[10px] font-black uppercase tracking-widest text-emerald-400 hover:text-brand-text transition-colors flex items-center gap-1.5">
                                <span className="text-lg leading-none">+</span> Vender minha cópia
                            </a>
                        </div>

                        {/* Table Header Row (Desktop) */}
                        <div className="hidden sm:flex items-center px-4 py-3 border-b border-white/5" style={{background:'#23293c'}}>
                            <div className="flex-[2] text-[9px] font-black uppercase tracking-widest text-brand-muted pl-14">Vendedor</div>
                            <div className="flex-1 text-[9px] font-black uppercase tracking-widest text-brand-muted text-center">Qualidade / Idioma</div>
                            <div className="flex-1 text-[9px] font-black uppercase tracking-widest text-brand-muted text-center">Extras</div>
                            <div className="flex-1 text-[9px] font-black uppercase tracking-widest text-brand-muted text-right pr-4">Preço Unitário</div>
                            <div className="flex-[1.5] text-[9px] font-black uppercase tracking-widest text-brand-muted text-right pr-4">Unidades e Compra</div>
                        </div>
