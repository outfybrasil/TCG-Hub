...
import { ArrowLeft, BellRing, ChevronUp, Clock3, Crown, History, Maximize2, Minimize2, Radio, RefreshCw, Share2, ShieldCheck, Users, Wallet, Wifi, WifiOff, X } from 'lucide-react';
    ...
    return <div className="min-h-dvh bg-[#070d1f] text-white lg:h-dvh lg:overflow-hidden">
        ...
        <section className="live-mobile-room relative h-dvh overflow-hidden bg-black lg:hidden">
            ...
            <div className="absolute inset-x-0 top-0 z-10 flex items-center justify-between p-4 pt-[max(1rem,env(safe-area-inset-top))]">
                <button onClick={() => router.push('/lives')} aria-label="Voltar" className="flex h-11 w-11 items-center justify-center rounded-full bg-black/50 backdrop-blur-md active:bg-black/70"><ArrowLeft className="h-5 w-5" /></button>
                ...
                <div className="flex gap-1.5"><button onClick={() => setVideoFill(fill => !fill)} title={videoFill ? 'Mostrar vídeo inteiro' : 'Preencher a tela'} aria-label={videoFill ? 'Mostrar vídeo inteiro' : 'Preencher a tela'} className="flex h-11 w-11 items-center justify-center rounded-full bg-black/50 backdrop-blur-md active:bg-black/70">{videoFill ? <Minimize2 className="h-5 w-5" /> : <Maximize2 className="h-5 w-5" />}</button><button onClick={reloadVideo} title="Recarregar somente o vídeo" aria-label="Recarregar vídeo" className={`flex h-11 w-11 items-center justify-center rounded-full bg-black/50 backdrop-blur-md active:bg-black/70 ${videoLoading ? 'text-amber-300' : connected ? 'text-emerald-300' : 'text-amber-300'}`}><RefreshCw className={`h-5 w-5 ${videoLoading ? 'animate-spin' : ''}`} /></button></div>
            </div>
            ...
            <div className="live-mobile-auction absolute bottom-0 left-0 right-16 z-10 p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
                {!waiting && !ended && <>
                    <div className="mb-3 flex items-end gap-3">
                        {live.current_item_image && <div className="relative h-16 w-12 shrink-0 overflow-hidden rounded-lg border border-white/20 bg-black/40"><Image unoptimized fill sizes="48px" src={live.current_item_image} alt={live.current_item_name || 'Lote'} className="object-contain" /></div>}
                        <div className="min-w-0"><p className="text-[9px] font-bold uppercase tracking-wider text-rose-200">{live.current_item_type || 'Lote em disputa'}</p><h2 className="line-clamp-2 text-lg font-black leading-tight drop-shadow-lg">{live.current_item_name}</h2>{live.current_item_description && <p className="mt-1 line-clamp-1 text-[11px] text-white/70">{live.current_item_description}</p>}<p className={`mt-1 flex items-center gap-1 truncate text-[10px] font-bold ${winning ? 'text-emerald-300' : 'text-white/75'}`}><Crown className="h-3 w-3 shrink-0 text-amber-300" />{winning ? 'Você está liderando' : live.winning_user_name ? `Liderando: ${live.winning_user_name}` : 'Nenhum lance ainda'}</p></div>
                    </div>
                    <div className="mb-2 flex items-center gap-2"><div className="rounded-xl bg-black/55 px-3 py-2 backdrop-blur-md"><p className="text-[9px] font-bold text-white/70">Maior lance</p><p className="text-xl font-black tabular-nums text-amber-300">{money(Number(live.current_bid))}</p></div><div className={`rounded-xl px-3 py-2 backdrop-blur-md ${secondsLeft <= 10 ? 'bg-rose-700/90' : 'bg-black/55'}`}><p className="text-[9px] font-bold text-white/70">Termina em</p><p className="font-mono text-xl font-black tabular-nums">{formatTime(secondsLeft)}</p></div></div>
            ...
            <section className="flex min-h-0 flex-col bg-[radial-gradient(circle_at_top,rgba(225,29,72,.10),transparent_45%)]">
                <div className="flex-1 overflow-y-auto p-4 sm:p-6">
                    {ended ? <State icon="📺" title="Transmissão encerrada" text="Os arremates continuarão disponíveis na sua conta." /> : waiting ? <State icon="🔨" title="Próximo lote em preparação" text="Você verá o item aqui assim que o apresentador iniciar a disputa." /> : <>
                        <div className="flex items-start gap-4"><div className="relative flex h-28 w-24 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-white/10 bg-white/5 sm:h-36 sm:w-28">{live.current_item_image ? <Image unoptimized fill sizes="112px" src={live.current_item_image} alt={live.current_item_name || 'Lote'} className="object-contain" /> : <span className="text-4xl">🎴</span>}</div><div className="min-w-0"><p className="text-[9px] font-black uppercase tracking-[.2em] text-rose-400">Lote em disputa · {live.current_item_type || 'TCG'}</p><h2 className="mt-2 text-2xl font-black leading-tight sm:text-3xl">{live.current_item_name}</h2>{live.current_item_description && <p className="mt-2 line-clamp-3 text-xs leading-5 text-slate-400">{live.current_item_description}</p>}</div></div>
